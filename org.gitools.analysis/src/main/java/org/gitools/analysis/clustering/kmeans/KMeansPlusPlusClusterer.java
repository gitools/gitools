...
public class KMeansPlusPlusClusterer<T extends Clusterable> {
    ...
    /**
     * Adds the given points to the closest {@link org.apache.commons.math3.ml.clustering.Cluster}.
     *
     * @param clusters the {@link org.apache.commons.math3.ml.clustering.Cluster}s to add the points to
     * @param points the points to add to the given {@link org.apache.commons.math3.ml.clustering.Cluster}s
    ...
    /**
     * Get a random point from the {@link org.apache.commons.math3.ml.clustering.Cluster} with the largest distance variance.
     *
     * @param clusters the {@link org.apache.commons.math3.ml.clustering.Cluster}s to search
    ...
    private Clusterable centroidOf(final Collection<T> points, final int dimension) {
        ...
        final List<Double> centroid = new ArrayList<>(dimension);

        // Intitialize centroid
        for (int i=0; i < dimension; i++) {
            centroid.add(0.0);
        }

        for (final T p : points) {
            final Iterator<Double> point = p.getPoint().iterator();

            int i = 0;
            while (point.hasNext()) {
                Double value = centroid.get(i);
                Double next = point.next();

                if (next != null) {
                    centroid.set(i, value + next);
                }
        ...
        for (int i = 0; i < centroid.size(); i++) {
            Double value = centroid.get(i);
            centroid.set(i, value / points.size());
        }
     ...
     * with the configured {@link org.apache.commons.math3.ml.distance.DistanceMeasure}.
     *
