...
public class FileFormats {
	...
	public static final FileFormat GENE_MATRIX = new FileFormat(
			"Gene Matrix (gmx)", FileSuffixes.GENE_MATRIX, MimeTypes.GENE_MATRIX);

	public static final FileFormat GENE_MATRIX_TRANSPOSED = new FileFormat(
			"Gene Matrix Transposed (gmt)", FileSuffixes.GENE_MATRIX_TRANSPOSED, MimeTypes.GENE_MATRIX_TRANSPOSED);

	public static final FileFormat DOUBLE_BINARY_MATRIX = new FileFormat(
			"Binary data matrix (bdm)", FileSuffixes.DOUBLE_BINARY_MATRIX, MimeTypes.DOUBLE_BINARY_MATRIX);

	public static final FileFormat DOUBLE_MATRIX = new FileFormat(
			"Continuous data matrix (cdm)", FileSuffixes.DOUBLE_MATRIX, MimeTypes.DOUBLE_MATRIX);

	public static final FileFormat MODULES_INDEXED_MAP = new FileFormat(
			"Indexed mappings (ixm)", FileSuffixes.MODULES_INDEXED_MAP, MimeTypes.MODULES_INDEXED_MAP);

	public static final FileFormat MODULES_2C_MAP = new FileFormat(
			"Two columns mappings (tcm)", FileSuffixes.MODULES_2C_MAP, MimeTypes.MODULES_2C_MAP);
