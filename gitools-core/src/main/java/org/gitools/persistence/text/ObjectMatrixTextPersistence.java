...
public class ObjectMatrixTextPersistence extends BaseMatrixPersistence<ObjectMatrix> {
    ...
    @Override
    public Map<String, Object> readMetadata(IResourceLocator resourceLocator, String[] keys, IProgressMonitor progressMonitor) throws PersistenceException {
        Map<String, Object> meta = new HashMap<String, Object>();

        for (String key : keys) {
            if (META_ATTRIBUTES.equals(key)) {
                meta.put(META_ATTRIBUTES, readMetaAttributes(resourceLocator, progressMonitor));
            }
        }
    ...
    private List<IElementAttribute> readMetaAttributes(IResourceLocator resourceLocator, IProgressMonitor progressMonitor) throws PersistenceException {

        IElementAdapter elementAdapter;
        Map<String, String> meta = readFormatAttributes(resourceLocator, progressMonitor);
    ...
    @Override
    public ObjectMatrix read(IResourceLocator resourceLocator, IProgressMonitor progressMonitor) throws PersistenceException {

        ObjectMatrix resultsMatrix = new ObjectMatrix();
        read(resourceLocator, resultsMatrix, progressMonitor);
        return resultsMatrix;
    }

    public void read(IResourceLocator resourceLocator, ObjectMatrix multiValMatrix, IProgressMonitor monitor) throws PersistenceException {

        monitor.begin("Loading results ...", 1);
        monitor.info("From: " + resourceLocator.getURL());

        Map<String, String> meta = readFormatAttributes(resourceLocator, monitor);
    ...
    @Override
    public void write(IResourceLocator resourceLocator, ObjectMatrix results, IProgressMonitor progressMonitor) throws PersistenceException {
        write(resourceLocator, results, true, progressMonitor);
    }

    public void write(IResourceLocator resourceLocator, ObjectMatrix results, boolean orderByColumn, IProgressMonitor monitor) throws PersistenceException {

        monitor.begin("Saving results...", results.getRowCount() * results.getColumnCount());
        monitor.info("To: " + resourceLocator.getURL());

        try {
            Writer writer = resourceLocator.getWriter();
    ...
    public void writeCells(Writer writer, ObjectMatrix resultsMatrix, boolean orderByColumn, IProgressMonitor progressMonitor) {
    ...
    private Map<String, String> readFormatAttributes(IResourceLocator resourceLocator, IProgressMonitor progressMonitor) throws PersistenceException {
        Map<String, String> meta = new HashMap<String, String>();
        try {
            BufferedReader r = new BufferedReader(resourceLocator.getReader());
            boolean done = false;
            String cl = null;
